...
import {
  menuPortfolioPresetLabels,
  type MenuPortfolioPreset
...
function buildPresetRange(preset: MenuPortfolioPreset, maxDateIso?: string) {
...
interface MenuPortfolioFiltersProps {
  maxDateIso?: string;
}

export function MenuPortfolioFilters({ maxDateIso }: MenuPortfolioFiltersProps) {
  const [mounted, setMounted] = React.useState(false);
  const [, startRefresh] = useTransition();
  const [params, setParams] = useQueryStates(menuPortfolioSearchParams, {
    history: 'replace',
    shallow: true,
    scroll: false,
    ...
    <div className='flex flex-wrap items-end gap-3'>
      <div className='flex flex-col gap-1'>
        ...
        <Select
          ...
          onValueChange={(value) => {
            const preset = value as MenuPortfolioPreset;
        ...
        >
          ...
          <SelectContent>
            {Object.entries(menuPortfolioPresetLabels).map(([value, label]) => (
