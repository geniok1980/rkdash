...
import { getWaiterRewardPercent, setWaiterRewardPercent } from '@/lib/rkeeper-data';

export const dynamic = 'force-dynamic';

export async function GET() {
  const percent = await getWaiterRewardPercent();
...
export async function POST(req: NextRequest) {
  ...
  const saved = await setWaiterRewardPercent(num);
